...
GLuint gen_cube_buffer(float x, float y, float z, float n, int w) {
    ...
    try {
        ...
        if (block->type() == Block::BLOCKTYPE_SLAB) {
            make_slab(data, ao, light, 1, 1, 1, 1, 1, 1, x, y, z, n, w);
        } else {
...
void draw_sign(ShaderAttributes *attrib, GLuint buffer, int length) {
    glEnable(GL_POLYGON_OFFSET_FILL);
    glPolygonOffset(-8, -1024);
...
void draw_cube(ShaderAttributes *attrib, GLuint buffer) {
    draw_item(attrib, buffer, 36);
}

void draw_plant(ShaderAttributes *attrib, GLuint buffer) {
    draw_item(attrib, buffer, 24);
}

void draw_player(ShaderAttributes *attrib, Player *player) {
...
/**
 * @discussion Checking if a chunk were at (p, q) would it be visible. Use
 * Chunk::isVisible() instead if the chunk is already created.
 * @param planes Frustum planes
 * @param p The position of the chunk in chunk coordinates (x)
 * @param q The position of the chunk in chunk coordinates (z)
 * @param miny The minimum view distance
...
bool chunk_visible(float planes[6][4], int p, int q, int miny, int maxy)
{
    ...
    int d = CHUNK_SIZE + 1;
    float points[8][3] = {
        {static_cast<float>(x + 0), static_cast<float>(miny), static_cast<float>(z + 0)},
        {static_cast<float>(x + d), static_cast<float>(miny), static_cast<float>(z + 0)},
        {static_cast<float>(x + 0), static_cast<float>(miny), static_cast<float>(z + d)},
        {static_cast<float>(x + d), static_cast<float>(miny), static_cast<float>(z + d)},
        {static_cast<float>(x + 0), static_cast<float>(maxy), static_cast<float>(z + 0)},
        {static_cast<float>(x + d), static_cast<float>(maxy), static_cast<float>(z + 0)},
        {static_cast<float>(x + 0), static_cast<float>(maxy), static_cast<float>(z + d)},
        {static_cast<float>(x + d), static_cast<float>(maxy), static_cast<float>(z + d)}
    };
    int n = g->ortho ? 4 : 6;
    for (int i = 0; i < n; i++) {
        int in = 0;
    ...
    int *bx, int *by, int *bz)
{
    ...
    for (auto i = 0; i < g->chunk_count; i++) {
        ...
        if (chunk->distanceTo(p, q) > 1) {
            continue;
        }
        int hx, hy, hz;
        int hw = _hit_test(chunk->getBlockMap(), 5.0f, previous,
...
void compute_chunk(WorkerItem *item) {
    ...
    MAP_FOR_EACH(map, ex, ey, ez, ew) {
        ...
        if (is_plant(ew)) {
            ...
            //TODO: Handle better letting developer know
            //for sure that there is a problem with the block
            try {
                Block *block = GlobalBlockMap->at(ew);
                if (block->type() == Block::BLOCKTYPE_BLOCK) {
                    make_cube(
                              data + offset, ao, light,
                              f1, f2, f3, f4, f5, f6,
                              ex, ey, ez, 0.5, ew);
                } else if (block->type() == Block::BLOCKTYPE_SLAB) {
                    make_slab(
                              data + offset, ao, light,
                              f1, f2, f3, f4, f5, f6,
                              ex, ey, ez, 0.5, ew);
                ...
                std::cout << "Failed to find block " << ew << std::endl;
                make_cube(
                          data + offset, ao, light,
                          f1, f2, f3, f4, f5, f6,
                          ex, ey, ez, 0.5, ew);
...
void gen_chunk_buffer(Chunk *chunk) {
    ...
    for (int dp = -1; dp <= 1; dp++) {
        for (int dq = -1; dq <= 1; dq++) {
            Chunk *other = chunk;
            if (dp || dq) {
                other = Chunk::findChunk(chunk->getChunkPosition().x + dp, chunk->getChunkPosition().z + dq);
            }
            if (other) {
                item->block_maps[dp + 1][dq + 1] = other->getBlockMap();
                item->light_maps[dp + 1][dq + 1] = other->getLightMap();
            }
            else {
...
void create_chunk(Chunk *chunk, int p, int q) {
    ...
    item->q = chunk->getChunkPosition().z;
    item->chunk = chunk;
    item->block_maps[1][1] = chunk->getBlockMap();
    item->light_maps[1][1] = chunk->getLightMap();
    load_chunk(item);
...
void check_workers() {
    for (int i = 0; i < WORKERS; i++) {
        ...
        if (worker->state == WORKER_DONE) {
            ...
            Chunk *chunk = Chunk::findChunk(item->p, item->q);
            if (chunk) {
                if (item->load) {
                    Map *block_map = item->block_maps[1][1];
                    Map *light_map = item->light_maps[1][1];
                    ...
                    map_copy(chunk->getBlockMap(), block_map);
                    map_copy(chunk->getLightMap(), light_map);
...
void ensure_chunks_worker(Player *player, Worker *worker) {
    ...
    for (int dp = -r; dp <= r; dp++) {
        for (int dq = -r; dq <= r; dq++) {
            ...
            int priority = 0;
            if (chunk) {
                priority = chunk->buffer && chunk->isDirty();
            }
    ...
    for (int dp = -1; dp <= 1; dp++) {
        for (int dq = -1; dq <= 1; dq++) {
            ...
            if (other) {
                ...
                map_copy(block_map, other->getBlockMap());
                Map *light_map = new Map(); //malloc(sizeof(Map));
                map_copy(light_map, other->getLightMap());
...
void record_block(int x, int y, int z, int w) {
    memcpy(&g->block1, &g->block0, sizeof(CBlock));
    g->block0.x = x;
    g->block0.y = y;
    g->block0.z = z;
    g->block0.w = w;
...
int render_chunks(ShaderAttributes *attrib, Player *player) {
    int result = 0;
    State *s = &player->state;
    ...
    for (auto i = 0; i < g->chunk_count; i++) {
        ...
        if (chunk->distanceTo(p, q) > g->render_radius) {
            continue;
        }
        if (!chunk->isVisible(planes))
        {
            continue;
        }
        chunk->render(attrib);
        result += chunk->render(attrib);
    }
    return result;
}
...
void render_signs(ShaderAttributes *attrib, Player *player) {
    ...
    for (auto it = 0; it < g->chunk_count; it++) {
        Chunk *chunk = g->chunks + it;
        if (chunk->distanceTo(p, q) > g->sign_radius) {
            continue;
        }
        if (!chunk->isVisible(planes))
        {
...
void render_sign(ShaderAttributes *attrib, Player *player) {
    ...
    GLfloat *data = malloc_faces(5, static_cast<int>(strlen(text)));
...
void render_players(ShaderAttributes *attrib, Player *player) {
    State *s = &player->state;
    float matrix[16];
    set_matrix_3d(
        matrix, g->width, g->height,
...
void render_sky(ShaderAttributes *attrib, Player *player, GLuint buffer) {
    State *s = &player->state;
    float matrix[16];
    set_matrix_3d(
        matrix, g->width, g->height,
...
void render_wireframe(ShaderAttributes *attrib, Player *player) {
    State *s = &player->state;
    float matrix[16];
    set_matrix_3d(
        matrix, g->width, g->height,
...
void render_gui(ShaderAttributes *attrib)
{
    float matrix[16];
    set_matrix_item(matrix, g->width, g->height, g->scale);
    glUseProgram(attrib->program);
...
void copy() {
    memcpy(&g->copy0, &g->block0, sizeof(CBlock));
    memcpy(&g->copy1, &g->block1, sizeof(CBlock));
}
...
void array(CBlock *b1, CBlock *b2, int xc, int yc, int zc) {
    if (b1->w != b2->w) {
        return;
    }
    int w = b1->w;
...
void cube(CBlock *b1, CBlock *b2, int fill) {
    if (b1->w != b2->w) {
        return;
    }
    int w = b1->w;
...
void sphere(CBlock *center, int radius, int fill, int fx, int fy, int fz) {
...
void cylinder(CBlock *b1, CBlock *b2, int radius, int fill) {
    if (b1->w != b2->w) {
        return;
    }
    int w = b1->w;
...
void on_left_click() {
    ...
    if (hy > 0 && hy < 256 && is_destructable(hw)) {
        try {
            Block *block = GlobalBlockMap->at(hw);
            block->setBlockPosition(hx, hy, hz);
            if (block->onDestroy() == false) {
                return;
            }
        } catch(const std::out_of_range &err) {}
...
void parse_buffer(char *buffer) {
    ...
    while (line) {
        ...
        if (sscanf(line, "R,%d,%d", &kp, &kq) == 2) {
            Chunk *chunk = Chunk::findChunk(kp, kq);
            if (chunk) {
                chunk->dirty(true);
            }
        }
...
int main(int argc, char **argv) {
    ...
    importer.import(GlobalBlockMap);
    g = &model;
    ...
    load_png_texture("textures/atlas.png", 1);
    ...
    load_png_texture("textures/sky.png", 1);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    load_png_texture("textures/sign.png", 1);
    ...
    while (running) {
        ...
        while (1) {
            ...
            if (g->prev_chunk_count < g->chunk_count)
                delete_chunks();
            ...
            if (SHOW_CHAT_TEXT) {
                for (int i = 0; i < MAX_MESSAGES; i++) {
                    ...
                    if (strlen(g->messages[index])) {
                        render_text(&text_attrib, ALIGN_LEFT, tx, ty, text_size,
                            g->messages[index]);
                        ty -= text_size * 2;
                    }
                }
            ...
            if (SHOW_PLAYER_NAMES) {
                if (player != me) {
                    render_text(&text_attrib, ALIGN_CENTER,
                        g->width / 2, text_size, text_size, player->name);
                }
                Player *other = player_crosshair(player);
                if (other) {
                    render_text(&text_attrib, ALIGN_CENTER,
                        g->width / 2, g->height / 2 - text_size - 24, text_size,
                        other->name);
                }
            }
            ...
            if (g->observe2) {
                ...
                if (SHOW_PLAYER_NAMES) {
                    render_text(&text_attrib, ALIGN_CENTER,
                        pw / 2, text_size, text_size, player->name);
                }
            }
