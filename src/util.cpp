...
char *load_file(const char *path) {
    ...
    rewind(file);
    char *data = new char[length + 1]();//TODO: Remove (char*)calloc(length + 1, sizeof(char));
...
GLfloat *malloc_faces(int components, int faces) {
    return new GLfloat[6 * components * faces];//TODO: REMOVE malloc(sizeof(GLfloat) * 6 * components * faces);
}
...
GLuint make_shader(GLenum type, const char *source) {
    ...
    if (status == GL_FALSE) {
        ...
        GLchar *info = new GLchar[length]();//TODO: remove calloc(length, sizeof(GLchar));
...
GLuint make_program(GLuint shader1, GLuint shader2) {
    ...
    if (status == GL_FALSE) {
        ...
        GLchar *info = new GLchar[length];//calloc(length, sizeof(GLchar));
    ...
    unsigned char *data, unsigned int width, unsigned int height)
{
    ...
    unsigned char *new_data = new unsigned char[size];//TODO: Remove malloc(sizeof(unsigned char) * size);
...
void load_png_texture(const char *file_name, bool flip_image) {
    unsigned error;
    unsigned char *data;
...
int wrap(const char *input, int max_width, char *output, int max_length) {
    *output = '\0';
    char *text = new char[strlen(input) + 1];//TODO: Remove malloc(sizeof(char) * (strlen(input) + 1));
    strcpy(text, input);
